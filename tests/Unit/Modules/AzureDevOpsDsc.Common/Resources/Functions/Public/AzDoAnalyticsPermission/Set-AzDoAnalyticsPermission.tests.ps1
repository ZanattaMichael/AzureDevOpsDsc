...
Describe "Set-AzDoAnalyticsPermission" -Tag "Unit", "AnalyticsPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoAnalyticsPermission.tests.ps1'
        }
    ...
    Context "when security namespace and project are found" {
        ...
        It "calls ConvertTo-ACLHashtable" {
            Set-AzDoAnalyticsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' `
    ...
    Context "when security namespace not found" {
        ...
        It "throws instead of returning silently, so a failed converge is never reported as success" {
            { Set-AzDoAnalyticsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false } | Should -Throw
