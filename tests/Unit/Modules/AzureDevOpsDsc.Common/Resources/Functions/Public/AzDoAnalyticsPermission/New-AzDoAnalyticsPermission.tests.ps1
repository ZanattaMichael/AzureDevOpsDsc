...
Describe 'New-AzDoAnalyticsPermission' -Tag "Unit", "AnalyticsPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoAnalyticsPermission.tests.ps1'
        }
    ...
    It 'calls ConvertTo-ACLHashtable' {
        New-AzDoAnalyticsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @() }
    ...
    Context 'when security namespace is not found' {
        ...
        It 'throws instead of returning silently, so a failed converge is never reported as success' {
            { New-AzDoAnalyticsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false } | Should -Throw
