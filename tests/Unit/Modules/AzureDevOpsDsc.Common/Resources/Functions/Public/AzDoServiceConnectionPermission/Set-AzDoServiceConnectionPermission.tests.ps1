...
Describe "Set-AzDoServiceConnectionPermission" -Tag "Unit", "ServiceConnectionPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoServiceConnectionPermission.tests.ps1'
        }
