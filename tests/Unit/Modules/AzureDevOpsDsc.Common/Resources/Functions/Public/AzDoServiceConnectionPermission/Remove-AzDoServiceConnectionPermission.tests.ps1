...
Describe "Remove-AzDoServiceConnectionPermission" -Tag "Unit", "ServiceConnectionPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Remove-AzDoServiceConnectionPermission.tests.ps1'
        }
