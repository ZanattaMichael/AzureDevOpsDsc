...
Describe "Set-AzDoQueryPermission" -Tag "Unit", "WorkItemQuery", "Permission" {

    BeforeAll {

        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoQueryPermission.tests.ps1'
        }
    ...
    Context "when a folder ACL token was resolved" {
        ...
        It "writes the permissions" {
            ...
            Set-AzDoQueryPermission -ProjectName 'TestProject' -QueryPath 'Shared Queries/Platform' -isInherited $true -LookupResult $lookup
    ...
    Context "when targeting the project query root" {
        ...
        It "clears ACEs, because the root has no parent to inherit from" {
            ...
            Set-AzDoQueryPermission -ProjectName 'TestProject' -isInherited $true -LookupResult $lookup
            Assert-MockCalled -CommandName Set-AzDoPermission -Exactly -Times 1 -ParameterFilter {
                $ClearACEs -eq $true
            }
        }
