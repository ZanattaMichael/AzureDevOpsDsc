...
Describe "Set-AzDoAnalyticsViewsPermission" -Tag "Unit", "AnalyticsViewsPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoAnalyticsViewsPermission.tests.ps1'
        }
        ...
        Mock -CommandName Clear-AzDoACE
    ...
    Context "when security namespace and project are found" {
        ...
        It "calls ConvertTo-ACLHashtable" {
            Set-AzDoAnalyticsViewsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' `
    ...
    Context "when security namespace not found" {
        ...
        It "throws instead of returning silently, so a failed converge is never reported as success" {
            { Set-AzDoAnalyticsViewsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false } | Should -Throw
