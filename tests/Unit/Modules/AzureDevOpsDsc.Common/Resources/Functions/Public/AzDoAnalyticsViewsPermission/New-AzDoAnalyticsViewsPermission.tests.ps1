...
Describe 'New-AzDoAnalyticsViewsPermission' -Tag "Unit", "AnalyticsViewsPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoAnalyticsViewsPermission.tests.ps1'
        }
    ...
    It 'calls ConvertTo-ACLHashtable' {
        New-AzDoAnalyticsViewsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @() }
    ...
    Context 'when security namespace is not found' {
        ...
        It 'throws instead of returning silently, so a failed converge is never reported as success' {
            { New-AzDoAnalyticsViewsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false } | Should -Throw
