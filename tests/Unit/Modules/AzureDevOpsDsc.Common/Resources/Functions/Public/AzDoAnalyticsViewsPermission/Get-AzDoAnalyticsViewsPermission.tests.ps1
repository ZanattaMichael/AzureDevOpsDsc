...
Describe 'Get-AzDoAnalyticsViewsPermission' -Tag "Unit", "AnalyticsViewsPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Get-AzDoAnalyticsViewsPermission.tests.ps1'
        }
        ...
        Mock -CommandName Get-DevOpsACL -MockWith {
            # token must match the format built in the source: '$/Shared/{id}'
            return @( @{ token = '$/Shared/mock-project-id'; ace = 'mock-ace' } )
        }

        Mock -CommandName ConvertTo-FormattedACL -MockWith {
            return @( @{ Token = @{ Type = 'AnalyticsViews'; ProjectId = 'mock-project-id' }; Permission = 'Allow' } )
        }
    ...
    Context 'when project and namespace are found' {
        ...
        It 'calls Get-DevOpsACL' {
            Get-AzDoAnalyticsViewsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false
        ...
        It 'returns a result with status from Test-ACLListforChanges' {
            $result = Get-AzDoAnalyticsViewsPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false
    ...
    Context 'when project is not found' {
        ...
        It 'returns status NotFound (not Missing, so Set/Remove is never invoked for an absent project)' {
            $result = Get-AzDoAnalyticsViewsPermission -ProjectName 'NonExistent' -GroupName 'TestGroup' -isInherited $false
            $result.status | Should -Be 'NotFound'
        }

        It 'does not call Get-DevOpsACL' {
            Get-AzDoAnalyticsViewsPermission -ProjectName 'NonExistent' -GroupName 'TestGroup' -isInherited $false
        ...
        It 'rethrows a live lookup failure that is not a 404' {
            ...
            { Get-AzDoAnalyticsViewsPermission -ProjectName 'NonExistent' -GroupName 'TestGroup' -isInherited $false } | Should -Throw '*401*'
        }
    }
