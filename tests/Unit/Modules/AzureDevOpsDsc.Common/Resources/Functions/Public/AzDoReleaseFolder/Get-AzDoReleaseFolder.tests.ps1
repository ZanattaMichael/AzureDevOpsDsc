...
Describe "Get-AzDoReleaseFolder" -Tag "Unit", "ReleaseFolder" {

    BeforeAll {

        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Get-AzDoReleaseFolder.tests.ps1'
        }
    ...
    Context "when the folder exists" {
        ...
        BeforeEach {
            Mock -CommandName List-DevOpsReleaseFolders -MockWith {
                return @(@{ path = '\Platform'; description = 'Platform releases' })
            }
        }

        It "returns status Unchanged when the description matches" {
            $result = Get-AzDoReleaseFolder -ProjectName 'TestProject' -Path '\Platform' -Description 'Platform releases'
            $result.status | Should -Be 'Unchanged'
        }

        It "returns status Changed when the description differs" {
            $result = Get-AzDoReleaseFolder -ProjectName 'TestProject' -Path '\Platform' -Description 'Something else'
        ...
        It "does not treat an unspecified description as drift" {
            ...
            $result = Get-AzDoReleaseFolder -ProjectName 'TestProject' -Path '\Platform'
            $result.status | Should -Be 'Unchanged'
        }
    ...
    Context "when the folder does not exist" {
        ...
        It "returns status NotFound" {
            $result = Get-AzDoReleaseFolder -ProjectName 'TestProject' -Path '\Missing'
    ...
    Context "when the path is the project release root" {
        ...
        It "refuses to manage it as a folder" {
            $result = Get-AzDoReleaseFolder -ProjectName 'TestProject' -Path '\'
