...
Describe "New-AzDoPipelinePermission" -Tag "Unit", "PipelinePermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoPipelinePermission.tests.ps1'
        }
    ...
    Context "when namespace and project are found" {
        ...
        It "calls ConvertTo-ACLHashtable" {
            New-AzDoPipelinePermission -ProjectName 'TestProject' -PipelineName 'TestPipeline' `
                -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @() }
