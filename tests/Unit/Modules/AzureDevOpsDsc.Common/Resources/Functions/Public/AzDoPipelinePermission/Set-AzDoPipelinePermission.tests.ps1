...
Describe "Set-AzDoPipelinePermission" -Tag "Unit", "PipelinePermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoPipelinePermission.tests.ps1'
        }
    ...
    Context "when namespace and project are found" {
        ...
        It "calls ConvertTo-ACLHashtable" {
            Set-AzDoPipelinePermission -ProjectName 'TestProject' -PipelineName 'TestPipeline' `
                -GroupName 'TestGroup' -isInherited $false `
                -LookupResult @{ propertiesChanged = @(); DifferenceACLs = @() }
            Assert-MockCalled -CommandName ConvertTo-ACLHashtable -Exactly -Times 1
        }

        It "calls Set-AzDoPermission with ClearACEs" {
            Set-AzDoPipelinePermission -ProjectName 'TestProject' -PipelineName 'TestPipeline' `
                -GroupName 'TestGroup' -isInherited $false `
                -LookupResult @{ propertiesChanged = @(); DifferenceACLs = @() }
