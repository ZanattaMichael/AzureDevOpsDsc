...
Describe "Set-AzDoTaggingPermission" -Tag "Unit", "TaggingPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoTaggingPermission.tests.ps1'
        }
    ...
    Context "when security namespace and project are found" {
        ...
        It "calls ConvertTo-ACLHashtable" {
            Set-AzDoTaggingPermission -ProjectName 'TestProject' -GroupName 'TestGroup' `
    ...
    Context "when security namespace not found" {
        ...
        It "throws instead of returning silently, so a failed converge is never reported as success" {
            { Set-AzDoTaggingPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false } | Should -Throw
