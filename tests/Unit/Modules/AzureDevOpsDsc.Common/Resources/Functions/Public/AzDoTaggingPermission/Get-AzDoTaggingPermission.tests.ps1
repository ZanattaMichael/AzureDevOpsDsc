...
Describe 'Get-AzDoTaggingPermission' -Tag "Unit", "TaggingPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Get-AzDoTaggingPermission.tests.ps1'
        }
        ...
        Mock -CommandName ConvertTo-FormattedACL -MockWith {
            return @( @{ Token = @{ Type = 'Tagging'; ProjectId = 'mock-project-id' }; Permission = 'Allow' } )
        }
    ...
    Context 'when project and namespace are found' {
        ...
        It 'calls Get-DevOpsACL' {
            Get-AzDoTaggingPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false
        ...
        It 'returns a result with status from Test-ACLListforChanges' {
            $result = Get-AzDoTaggingPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false
