...
Describe 'New-AzDoTaggingPermission' -Tag "Unit", "TaggingPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoTaggingPermission.tests.ps1'
        }
    ...
    It 'calls ConvertTo-ACLHashtable' {
        New-AzDoTaggingPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @() }
    ...
    Context 'when security namespace is not found' {
        ...
        It 'throws instead of returning silently, so a failed converge is never reported as success' {
            { New-AzDoTaggingPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false } | Should -Throw
