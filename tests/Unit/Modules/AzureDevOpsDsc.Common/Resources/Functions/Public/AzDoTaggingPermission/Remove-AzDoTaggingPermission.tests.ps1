...
Describe "Remove-AzDoTaggingPermission" -Tag "Unit", "TaggingPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Remove-AzDoTaggingPermission.tests.ps1'
        }
    ...
    Context "when security namespace is found" {
        ...
        It "calls Remove-AzDoPermission" {
            Remove-AzDoTaggingPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false
    ...
    Context "when security namespace not found" {
        ...
        It "throws instead of returning silently, so a failed converge is never reported as success" {
            { Remove-AzDoTaggingPermission -ProjectName 'TestProject' -GroupName 'TestGroup' -isInherited $false } | Should -Throw
