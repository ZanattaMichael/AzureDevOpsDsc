...
Describe "Get-AzDoSecurityNamespacePermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Get-AzDoSecurityNamespacePermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            Get-AzDoSecurityNamespacePermission -SecurityNamespace 'TestNS' -Token 'TestToken' -GroupName 'TestGroup' -isInherited $false
