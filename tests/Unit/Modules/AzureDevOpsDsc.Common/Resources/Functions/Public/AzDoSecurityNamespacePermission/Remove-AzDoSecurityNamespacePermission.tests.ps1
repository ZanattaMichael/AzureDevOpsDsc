...
Describe "Remove-AzDoSecurityNamespacePermission" -Tag "Unit", "SecurityNamespacePermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Remove-AzDoSecurityNamespacePermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            Remove-AzDoSecurityNamespacePermission -SecurityNamespace 'TestNS' -Token 'TestToken' -GroupName 'TestGroup' -isInherited $false
