...
Describe "Set-AzDoSecurityNamespacePermission" -Tag "Unit", "SecurityNamespacePermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoSecurityNamespacePermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            Set-AzDoSecurityNamespacePermission -SecurityNamespace 'TestNS' -Token 'TestToken' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @(); DifferenceACLs = @() }
    ...
    Context "when namespace not found" {
        ...
        It "writes an error" {
            Set-AzDoSecurityNamespacePermission -SecurityNamespace 'TestNS' -Token 'TestToken' -GroupName 'TestGroup' -isInherited $false
