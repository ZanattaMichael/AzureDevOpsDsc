...
Describe "New-AzDoReleaseDefinitionPermission" -Tag "Unit", "ReleaseDefinition", "Permission" {

    BeforeAll {

        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoReleaseDefinitionPermission.tests.ps1'
        }
    ...
    Context "when the namespace and token resolve" {
        ...
        It "writes the ACL and invalidates the cache" {
            $lookupResult = @{ aclToken = 'proj-id-1/123'; propertiesChanged = @(); DifferenceACLs = @() }
            New-AzDoReleaseDefinitionPermission -ProjectName 'TestProject' -DefinitionName 'MyRelease' -isInherited $true -LookupResult $lookupResult
    ...
    Context "when the security namespace cannot be found" {
        ...
        It "throws rather than reporting success" {
            $lookupResult = @{ aclToken = 'proj-id-1/123' }
            { New-AzDoReleaseDefinitionPermission -ProjectName 'TestProject' -DefinitionName 'MyRelease' -isInherited $true -LookupResult $lookupResult } | Should -Throw
