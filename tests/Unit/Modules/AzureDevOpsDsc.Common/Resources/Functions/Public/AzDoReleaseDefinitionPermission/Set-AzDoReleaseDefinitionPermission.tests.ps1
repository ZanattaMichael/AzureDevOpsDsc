...
Describe "Set-AzDoReleaseDefinitionPermission" -Tag "Unit", "ReleaseDefinition", "Permission" {

    BeforeAll {

        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoReleaseDefinitionPermission.tests.ps1'
        }
    ...
    Context "when the namespace and token resolve" {
        ...
        It "writes the ACL and invalidates the cache" {
            ...
            Set-AzDoReleaseDefinitionPermission -ProjectName 'TestProject' -DefinitionName 'MyRelease' -isInherited $true -LookupResult $lookupResult
    ...
    Context "when the security namespace cannot be found" {
        ...
        It "throws rather than reporting success" {
            ...
            { Set-AzDoReleaseDefinitionPermission -ProjectName 'TestProject' -DefinitionName 'MyRelease' -isInherited $true -LookupResult $lookupResult } | Should -Throw
