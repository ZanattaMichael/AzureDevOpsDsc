...
Describe "New-AzDoVariableGroupPermission" -Tag "Unit", "VariableGroupPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoVariableGroupPermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            New-AzDoVariableGroupPermission -ProjectName 'TestProject' -VariableGroupName 'TestVG' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @(); DifferenceACLs = @() }
    ...
    Context "when namespace not found" {
        ...
        It "writes an error" {
            New-AzDoVariableGroupPermission -ProjectName 'TestProject' -VariableGroupName 'TestVG' -GroupName 'TestGroup' -isInherited $false
