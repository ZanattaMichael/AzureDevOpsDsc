...
Describe "Remove-AzDoVariableGroupPermission" -Tag "Unit", "VariableGroupPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Remove-AzDoVariableGroupPermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            Remove-AzDoVariableGroupPermission -ProjectName 'TestProject' -VariableGroupName 'TestVG' -GroupName 'TestGroup' -isInherited $false
