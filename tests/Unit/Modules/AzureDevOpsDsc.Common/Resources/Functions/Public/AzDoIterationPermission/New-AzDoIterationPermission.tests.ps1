...
Describe 'New-AzDoIterationPermission Tests' {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoIterationPermission.tests.ps1'
        }
    ...
    Context "When Ensure is Present" {
        ...
        It "Should return warning when Security Namespace or Project is not found" {
            ...
            # Act
            $result = New-AzDoIterationPermission -ProjectName "NonExistentProject" -IterationPath "TestAreaPath" -isInherited $false
        ...
        It "Should set permissions correctly with valid parameters" {
            ...
            # Act
            $result = New-AzDoIterationPermission -ProjectName "TestProject" -IterationPath "ValidAreaPath" -isInherited $true -LookupResult @{
                propertiesChanged = @{
