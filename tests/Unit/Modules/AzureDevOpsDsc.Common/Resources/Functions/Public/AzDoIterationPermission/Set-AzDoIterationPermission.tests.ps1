...
Describe 'Set-AzDoIterationPermission Tests' {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoIterationPermission.tests.ps1'
        }
    ...
    Context "When AreaPath is not specified" {
        It "Should write a warning and stop execution" {
            ...
            { Set-AzDoIterationPermission -ProjectName "TestProject" -isInherited $true } |
                Should -Not -Throw
        }
    }
    ...
    Context "When Security Namespace is not found" {
        ...
        It "Should write an error message" {
            ...
            { Set-AzDoIterationPermission -ProjectName "TestProject" -IterationPath "SampleAreaPath" -isInherited $true } |
                Should -Not -Throw
        }
    }
    ...
    Context "When AreaPath is not specified" {
        It "Should set ClearACEs to true and call Set-AzDoPermission" {
            ...
            Set-AzDoIterationPermission -ProjectName "SampleProject" -isInherited $true -LookupResult $LookupResult
    ...
    Context "When AreaPath is specified" {
        It "Should not set ClearACEs and call Set-AzDoPermission" {
            ...
            Set-AzDoIterationPermission -ProjectName "SampleProject" -IterationPath "SampleAreaPath" -isInherited $true -LookupResult $LookupResult
