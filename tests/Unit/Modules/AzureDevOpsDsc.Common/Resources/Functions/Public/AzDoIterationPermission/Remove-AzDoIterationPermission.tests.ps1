...
Describe "Remove-AzDoIterationPermission Tests" -Tag "Unit", "IterationPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Remove-AzDoIterationPermission.tests.ps1'
        }
    ...
    Context "When AreaPath is not specified" {
        It "Should write a warning and stop execution" {
            ...
            { Remove-AzDoIterationPermission -ProjectName "TestProject" -isInherited $true } |
                Should -Not -Throw
        }
    }
    ...
    Context "When Security Namespace is not found" {
        ...
        It "Should write an error message" {
            ...
            { Remove-AzDoIterationPermission -ProjectName "TestProject" -IterationPath "SampleAreaPath" -isInherited $true } |
                Should -Not -Throw
        }
    }
