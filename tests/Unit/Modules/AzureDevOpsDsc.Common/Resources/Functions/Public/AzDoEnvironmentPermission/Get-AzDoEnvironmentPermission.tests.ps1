...
Describe "Get-AzDoEnvironmentPermission" -Tag "Unit", "EnvironmentPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Get-AzDoEnvironmentPermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            Get-AzDoEnvironmentPermission -ProjectName 'TestProject' -EnvironmentName 'TestEnv' -GroupName 'TestGroup' -isInherited $false
