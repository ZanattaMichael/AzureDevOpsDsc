...
Describe "New-AzDoEnvironmentPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoEnvironmentPermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            New-AzDoEnvironmentPermission -ProjectName 'TestProject' -EnvironmentName 'TestEnv' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @(); DifferenceACLs = @() }
    ...
    Context "when namespace not found" {
        ...
        It "writes an error" {
            New-AzDoEnvironmentPermission -ProjectName 'TestProject' -EnvironmentName 'TestEnv' -GroupName 'TestGroup' -isInherited $false
