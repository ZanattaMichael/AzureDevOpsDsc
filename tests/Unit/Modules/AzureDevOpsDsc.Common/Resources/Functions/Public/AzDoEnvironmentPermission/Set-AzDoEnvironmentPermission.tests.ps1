...
Describe "Set-AzDoEnvironmentPermission" -Tag "Unit", "EnvironmentPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoEnvironmentPermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            Set-AzDoEnvironmentPermission -ProjectName 'TestProject' -EnvironmentName 'TestEnv' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @(); DifferenceACLs = @() }
    ...
    Context "when namespace not found" {
        ...
        It "writes an error" {
            Set-AzDoEnvironmentPermission -ProjectName 'TestProject' -EnvironmentName 'TestEnv' -GroupName 'TestGroup' -isInherited $false
