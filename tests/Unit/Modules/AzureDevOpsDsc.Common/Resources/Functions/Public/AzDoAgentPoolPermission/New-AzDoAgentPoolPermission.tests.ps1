...
Describe "New-AzDoAgentPoolPermission" -Tag "Unit", "AgentPoolPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'New-AzDoAgentPoolPermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            New-AzDoAgentPoolPermission -PoolName 'TestPool' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @(); DifferenceACLs = @() }
    ...
    Context "when namespace not found" {
        ...
        It "writes an error" {
            New-AzDoAgentPoolPermission -PoolName 'TestPool' -GroupName 'TestGroup' -isInherited $false
