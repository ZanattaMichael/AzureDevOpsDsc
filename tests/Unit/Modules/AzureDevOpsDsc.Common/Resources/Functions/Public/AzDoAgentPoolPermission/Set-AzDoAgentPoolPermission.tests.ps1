...
Describe "Set-AzDoAgentPoolPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Set-AzDoAgentPoolPermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            Set-AzDoAgentPoolPermission -PoolName 'TestPool' -GroupName 'TestGroup' -isInherited $false -LookupResult @{ propertiesChanged = @(); DifferenceACLs = @() }
    ...
    Context "when namespace not found" {
        ...
        It "writes an error" {
            Set-AzDoAgentPoolPermission -PoolName 'TestPool' -GroupName 'TestGroup' -isInherited $false
