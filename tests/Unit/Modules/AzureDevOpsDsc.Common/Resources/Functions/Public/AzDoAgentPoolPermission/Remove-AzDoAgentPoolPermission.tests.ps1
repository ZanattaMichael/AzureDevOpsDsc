...
Describe "Remove-AzDoAgentPoolPermission" -Tag "Unit", "AgentPoolPermission" {
    ...
    BeforeAll {
        ...
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'Remove-AzDoAgentPoolPermission.tests.ps1'
        }
    ...
    Context "when namespace is found" {
        ...
        It "performs the expected operation" {
            Remove-AzDoAgentPoolPermission -PoolName 'TestPool' -GroupName 'TestGroup' -isInherited $false
