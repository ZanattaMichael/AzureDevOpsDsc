...
Describe "Format-AzDoIteration Tests" -Tag "Unit", "Helper" {

    BeforeAll {

        # Load the functions to test
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath "Format-AzDoIteration.tests.ps1"
        }
    ...
    Context "When called with valid parameters" {
        ...
        It "Should format path correctly when no leading slash is present" {
            ...
            $result = Format-AzDoIteration -Iteration $iteration -ProjectName $projectName
    ...
    Context "When called with explicit parameters" {
        ...
        It "Should format path correctly when no leading slash is present when defining the -StructureType is 'area'" {
            ...
            $result = Format-AzDoIteration -Iteration $iteration -ProjectName $projectName -StructureType 'Area'
        ...
        It "Should format path correctly when no leading slash is present when defining the -StructureType is 'Iteration'" {
            ...
            $result = Format-AzDoIteration -Iteration $iteration -ProjectName $projectName -StructureType 'Iteration'
