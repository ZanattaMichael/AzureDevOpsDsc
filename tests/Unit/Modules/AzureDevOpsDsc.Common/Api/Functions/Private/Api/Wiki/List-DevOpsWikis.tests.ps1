...
Describe 'List-DevOpsWikis' -Tag "Unit", "Wiki", "API" {

    BeforeAll {
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'List-DevOpsWikis.tests.ps1'
        }
    ...
    It 'Returns data when API returns results' {
        $result = List-DevOpsWikis -ApiUri 'https://dev.azure.com/myorg'
    ...
    It 'Calls Invoke-AzDevOpsApiRestMethod with GET method' {
        List-DevOpsWikis -ApiUri 'https://dev.azure.com/myorg'
    ...
    It 'Throws when the API call fails' {
        ...
        { List-DevOpsWikis -ApiUri 'https://dev.azure.com/myorg' } | Should -Throw
    }
