...
Describe 'List-DevOpsTaskGroups' -Tag "Unit", "TaskGroup", "API" {
    ...
    BeforeAll {
        if ($null -eq $currentFile) {
            $currentFile = Join-Path -Path $PSScriptRoot -ChildPath 'List-DevOpsTaskGroups.tests.ps1'
        }
    ...
    It 'Returns data when API returns results' {
        $result = List-DevOpsTaskGroups -ApiUri 'https://dev.azure.com/myorg' -ProjectName 'TestProject'
    ...
    It 'Calls Invoke-AzDevOpsApiRestMethod with GET method' {
        List-DevOpsTaskGroups -ApiUri 'https://dev.azure.com/myorg' -ProjectName 'TestProject'
    ...
    It 'Throws when the API call fails' {
        ...
        { List-DevOpsTaskGroups -ApiUri 'https://dev.azure.com/myorg' -ProjectName 'TestProject' } | Should -Throw
    }
