...
Describe 'AzDoAreaNodes Tests' -Tag "Unit", "Resources" {
    ...
    Context 'When getting the current state' {
        ...
        BeforeAll {
            Mock -CommandName Get-AzDoAreaNodes -ModuleName AzureDevOpsDscNative -MockWith {
                return @{
                    Ensure = [Ensure]::Absent
                    propertiesChanged = @()
                    ProjectName = "MyProject"
                    AreaPaths = @('AreaPath1', 'AreaPath2')
                    LookupResult = @{
                        Value = 1
                    }
                }
        ...
        It 'Should return the current state properties' {
            # Arrange
            $object = [AzDoAreaNodes]::new()
            $object.ProjectName = "MyProject"
            $object.AreaPaths = @('AreaPath1', 'AreaPath2', 'AreaPath3')
            ...
            $currentState.AreaPaths = @('AreaPath1', 'AreaPath2', 'AreaPath3')
        }
    }
