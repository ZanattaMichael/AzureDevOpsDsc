Function Get-AzDoPipelinePermission
{
    ...
    # The fallback above fires whenever the pipeline has no explicit ACL - the normal state
    # once permissions revert to inherited - so the full namespace is the common path, not a rare
    # one. The Build pattern is anchored, so the parsed filter below can only keep a
