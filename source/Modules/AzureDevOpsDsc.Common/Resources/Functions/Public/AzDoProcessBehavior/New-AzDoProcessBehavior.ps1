...
.EXAMPLE
New-AzDoProcessBehavior -ProcessName 'Contoso Agile' -WorkItemTypeName 'Incident' -BehaviorName 'Stories'
#>
Function New-AzDoProcessBehavior
{
    [CmdletBinding()]
    ...
    Write-Verbose "[New-AzDoProcessBehavior] Started."

    if ($LookupResult.reason -eq 'BehaviorNotFound')
    {
        Write-Error "[New-AzDoProcessBehavior] Behavior '$BehaviorName' does not exist on process '$ProcessName'. No change was made."
        return
    }
    ...
    {
        Write-Error "[New-AzDoProcessBehavior] Could not resolve work item type '$WorkItemTypeName' or behavior '$BehaviorName' on process '$ProcessName'."
        return
    }

    Write-Verbose "[New-AzDoProcessBehavior] Associating '$WorkItemTypeName' with backlog level '$BehaviorName'."
