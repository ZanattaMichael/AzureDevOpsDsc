...
.EXAMPLE
New-AzDoQueryPermission -ProjectName 'Contoso' -QueryPath 'Shared Queries/Platform' -isInherited $true
#>
Function New-AzDoQueryPermission
{
    [CmdletBinding()]
    ...
    Write-Verbose "[New-AzDoQueryPermission] Started."

    $SecurityNamespace = Get-CacheItem -Key 'WorkItemQueryFolders' -Type 'SecurityNamespaces'

    if ($null -eq $SecurityNamespace)
    {
        Write-Error "[New-AzDoQueryPermission] Security Namespace 'WorkItemQueryFolders' not found."
        return
    }
    ...
    if ($null -eq $Project)
    {
        Write-Error "[New-AzDoQueryPermission] Project not found: $ProjectName"
        return
    }
    ...
    {
        Write-Error "[New-AzDoQueryPermission] No ACL token was resolved for '$QueryPath' in project '$ProjectName'. Nothing was changed."
        return
    }
    ...
    {
        Write-Verbose "[New-AzDoQueryPermission] Targeting the project query root. Clearing ACEs."
        $params.ClearACEs       = $true
        $params.DifferenceACLs  = $LookupResult.DifferenceACLs
    }

    Write-Verbose "[New-AzDoQueryPermission] Setting query folder permissions for '$ProjectName' - '$token'."
