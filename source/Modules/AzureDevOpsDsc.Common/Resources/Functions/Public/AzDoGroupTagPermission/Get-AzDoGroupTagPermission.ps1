...
Function Get-AzDoGroupTagPermission
{
    [CmdletBinding()]
    ...
    Write-Verbose "[Get-AzDoGroupTagPermission] Started."
    ...
    if ($split.Count -ne 2)
    {
        Write-Warning "[Get-AzDoGroupTagPermission] Invalid Group Name: $GroupName"
        return
    }
    ...
    Write-Verbose "[Get-AzDoGroupTagPermission] Security Namespace: $SecurityNamespace"
    Write-Verbose "[Get-AzDoGroupTagPermission] Organization Name: $OrganizationName"
    ...
    Write-Verbose "[Get-AzDoGroupTagPermission] Group result hashtable constructed."
    Write-Verbose "[Get-AzDoGroupTagPermission] Performing lookup of permissions for the group."
    ...
    if (-not $group)
    {
        Throw "[Get-AzDoGroupTagPermission] Group not found: $('[{0}]\{1}' -f $ProjectName, $GroupName)"
        return
    }
    ...
    Write-Verbose "[Get-AzDoGroupTagPermission] Retrieved namespace: $($namespace.namespaceId)"
    ...
    Write-Verbose "[Get-AzDoGroupTagPermission] ACL Lookup Params: $($ACLLookupParams | Out-String)"
    ...
    Write-Verbose "[Get-AzDoGroupTagPermission] ACL List retrieved and formatted."
    ...
    {
        Write-Verbose "[Get-AzDoGroupTagPermission] No ACEs found for the group."
        return
    }
    ...
    # Write
    Write-Verbose "[Get-AzDoGroupTagPermission] Result Status: $($getGroupResult.status)"
    Write-Verbose "[Get-AzDoGroupTagPermission] Returning Group Result."
