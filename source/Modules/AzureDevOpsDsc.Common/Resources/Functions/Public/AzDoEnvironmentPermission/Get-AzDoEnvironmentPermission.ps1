Function Get-AzDoEnvironmentPermission
{
    ...
    # The fallback above fires whenever the environment has no explicit ACL - the normal state
    # once permissions revert to inherited - so the full namespace is the common path, not a rare
    # one. The Environment pattern is anchored, so the parsed filter below can only keep a
