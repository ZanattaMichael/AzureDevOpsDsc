Function New-AzDoEnvironmentPermission
{
    [CmdletBinding()]
    param (
    ...
    Write-Verbose "[New-AzDoEnvironmentPermission] Started."
    $SecurityNamespace = Get-CacheItem -Key 'DistributedTask' -Type 'SecurityNamespaces'
    $Project           = Get-CacheItem -Key $ProjectName -Type 'LiveProjects'
    if ((-not $SecurityNamespace) -or (-not $Project)) { Write-Error "[New-AzDoEnvironmentPermission] Cache miss."; return }
    $serializeACLParams = @{
