Function Get-AzDoVariableGroupPermission
{
    ...
    # The fallback above fires whenever the variable group has no explicit ACL - the normal state
    # once permissions revert to inherited - so the full namespace is the common path, not a rare
    # one. The Library pattern is anchored, so the parsed filter below can only keep a
