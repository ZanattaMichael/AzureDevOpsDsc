Function Get-AzDoServiceConnectionPermission
{
    ...
    if (-not $namespace) { Write-Error "[Get-AzDoServiceConnectionPermission] Security namespace not found." -ErrorAction Continue; $getResult.status = [DSCGetSummaryState]::Error; return $getResult }

    $getResult.namespace = $namespace

    # Token-scope the ACL fetch to this service connection's token instead of scanning the whole namespace.
    # Fall back to the full-namespace fetch if the scoped query returns nothing (never worse than before).
    $aclToken   = if ($scCache) { 'endpoints/Project/{0}/endpoint/{1}' -f $projectCache.id, $scCache.id } else { 'endpoints/Project/{0}' -f $projectCache.id }
    ...
    # The fallback above fires whenever the service connection has no explicit ACL - the normal state
    # once permissions revert to inherited - so the full namespace is the common path, not a rare
    # one. The ServiceEndpoint pattern is anchored, so the parsed filter below can only keep a
    # token equal to $aclToken: this drops exactly what that filter would have dropped.
