Function Remove-AzDoArtifactFeed
{
    [CmdletBinding()]
    param (
    ...
    Write-Verbose "[Remove-AzDoArtifactFeed] Removing feed '$FeedName'."
    $feed = Get-CacheItem -Key ('{0}\{1}' -f $ProjectName, $FeedName) -Type 'LiveArtifactFeeds'
    if (-not $feed) { Write-Error "[Remove-AzDoArtifactFeed] Feed not found."; return }
    $params = @{
        ApiUri      = 'https://feeds.dev.azure.com/{0}/' -f (Get-AzDoOrganizationName)
        ProjectName = $ProjectName
        FeedId      = $feed.id
    }
    Remove-DevOpsArtifactFeed @params
    Remove-CacheItem -Key ('{0}\{1}' -f $ProjectName, $FeedName) -Type 'LiveArtifactFeeds'
