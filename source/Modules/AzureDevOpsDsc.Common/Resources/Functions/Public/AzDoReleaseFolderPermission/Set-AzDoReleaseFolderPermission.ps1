...
.EXAMPLE
Set-AzDoReleaseFolderPermission -ProjectName 'Contoso' -FolderPath '\Platform' -isInherited $true
#>
Function Set-AzDoReleaseFolderPermission
{
    [CmdletBinding()]
    ...
    Write-Verbose "[Set-AzDoReleaseFolderPermission] Started."

    $SecurityNamespace = Get-CacheItem -Key 'ReleaseManagement' -Type 'SecurityNamespaces'

    if ($null -eq $SecurityNamespace)
    {
        throw "[Set-AzDoReleaseFolderPermission] Security Namespace 'ReleaseManagement' not found."
    }
    ...
    {
        throw "[Set-AzDoReleaseFolderPermission] No ACL token was resolved for '$FolderPath' in project '$ProjectName'. Nothing was changed."
    }
    ...
    {
        Write-Verbose "[Set-AzDoReleaseFolderPermission] Targeting the project release root. Clearing ACEs."
        $params.ClearACEs      = $true
        $params.DifferenceACLs = $LookupResult.DifferenceACLs
    }

    Write-Verbose "[Set-AzDoReleaseFolderPermission] Setting release folder permissions for token '$token'."
