...
.EXAMPLE
Set-AzDoSecureFilePermission -ProjectName 'Contoso' -SecureFileName 'signing.pfx' -isInherited $true
#>
Function Set-AzDoSecureFilePermission
{
    [CmdletBinding()]
    ...
    Write-Verbose "[Set-AzDoSecureFilePermission] Started."

    $SecurityNamespace = Get-CacheItem -Key 'Library' -Type 'SecurityNamespaces'

    if ($null -eq $SecurityNamespace)
    {
        Write-Error "[Set-AzDoSecureFilePermission] Security Namespace 'Library' not found."
        return
    }
    ...
    {
        Write-Error "[Set-AzDoSecureFilePermission] No ACL token was resolved for '$SecureFileName' in project '$ProjectName'. Nothing was changed."
        return
    }
    ...
    {
        Write-Verbose "[Set-AzDoSecureFilePermission] Targeting the project Library root. Clearing ACEs."
        $params.ClearACEs      = $true
        $params.DifferenceACLs = $LookupResult.DifferenceACLs
    }

    Write-Verbose "[Set-AzDoSecureFilePermission] Setting secure file permissions for token '$token'."
