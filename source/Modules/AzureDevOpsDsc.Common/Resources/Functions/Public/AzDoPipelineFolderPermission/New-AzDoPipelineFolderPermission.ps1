...
.EXAMPLE
New-AzDoPipelineFolderPermission -ProjectName 'Contoso' -FolderPath '\Platform' -isInherited $true
#>
Function New-AzDoPipelineFolderPermission
{
    [CmdletBinding()]
    ...
    Write-Verbose "[New-AzDoPipelineFolderPermission] Started."

    $SecurityNamespace = Get-CacheItem -Key 'Build' -Type 'SecurityNamespaces'

    if ($null -eq $SecurityNamespace)
    {
        Write-Error "[New-AzDoPipelineFolderPermission] Security Namespace 'Build' not found."
        return
    }
    ...
    {
        Write-Error "[New-AzDoPipelineFolderPermission] No ACL token was resolved for '$FolderPath' in project '$ProjectName'. Nothing was changed."
        return
    }
    ...
    {
        Write-Verbose "[New-AzDoPipelineFolderPermission] Targeting the project build root. Clearing ACEs."
        $params.ClearACEs      = $true
        $params.DifferenceACLs = $LookupResult.DifferenceACLs
    }

    Write-Verbose "[New-AzDoPipelineFolderPermission] Setting pipeline folder permissions for token '$token'."
