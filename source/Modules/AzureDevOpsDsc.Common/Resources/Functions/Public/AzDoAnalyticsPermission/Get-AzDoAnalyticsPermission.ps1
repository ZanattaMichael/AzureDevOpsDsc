Function Get-AzDoAnalyticsPermission
{
    [CmdletBinding()]
    ...
    Write-Verbose "[Get-AzDoAnalyticsPermission] Started."
    ...
    if (-not $projectCache)
    {
        Write-Verbose "[Get-AzDoAnalyticsPermission] Project '$ProjectName' not in cache — falling back to live API lookup."
    ...
    if (-not $namespace)
    {
        Write-Error "[Get-AzDoAnalyticsPermission] Security namespace not found." -ErrorAction Continue
        $getResult.status = [DSCGetSummaryState]::Error
    ...
    # Analytics tokens address the project by id: '/{projectId}'.
    ...
    $ReferenceACLs = @(ConvertTo-ACL @params | Where-Object { $_.token.Type -ne 'AnalyticsUnknown' })

    # The Analytics namespace has protected system-group ACEs that Azure DevOps auto-creates and
