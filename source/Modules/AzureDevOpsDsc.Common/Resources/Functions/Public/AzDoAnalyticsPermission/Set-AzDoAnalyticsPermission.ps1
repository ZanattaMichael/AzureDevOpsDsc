Function Set-AzDoAnalyticsPermission
{
    [CmdletBinding()]
    param (
    ...
    Write-Verbose "[Set-AzDoAnalyticsPermission] Started."
    ...
    if (-not $Project)
    {
        Write-Verbose "[Set-AzDoAnalyticsPermission] Project '$ProjectName' not in cache — falling back to live API lookup."
    ...
    {
        throw "[Set-AzDoAnalyticsPermission] Security namespace or project not found. Cannot converge on '$ProjectName'."
    }

    $serializeACLParams = @{
        ReferenceACLs        = $LookupResult.propertiesChanged
        DescriptorACLList    = @()
        DescriptorMatchToken = ($LocalizedDataAzSerializationPatten.AnalyticsPermission -f $Project.id)
    }
