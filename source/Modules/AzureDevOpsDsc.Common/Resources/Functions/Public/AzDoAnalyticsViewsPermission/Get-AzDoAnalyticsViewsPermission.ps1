Function Get-AzDoAnalyticsViewsPermission
{
    [CmdletBinding()]
    ...
    Write-Verbose "[Get-AzDoAnalyticsViewsPermission] Started."
    ...
    if (-not $projectCache)
    {
        Write-Verbose "[Get-AzDoAnalyticsViewsPermission] Project '$ProjectName' not in cache — falling back to live API lookup."
    ...
    if (-not $namespace)
    {
        Write-Error "[Get-AzDoAnalyticsViewsPermission] Security namespace not found." -ErrorAction Continue
        $getResult.status = [DSCGetSummaryState]::Error
    ...
    # AnalyticsViews tokens address the project by id: '/{projectId}'.
    $projectToken = '$/Shared/{0}' -f $projectCache.id
    ...
    $ReferenceACLs = @(ConvertTo-ACL @params | Where-Object { $_.token.Type -ne 'AnalyticsViewsUnknown' })

    # The AnalyticsViews namespace has protected system-group ACEs that Azure DevOps auto-creates and
