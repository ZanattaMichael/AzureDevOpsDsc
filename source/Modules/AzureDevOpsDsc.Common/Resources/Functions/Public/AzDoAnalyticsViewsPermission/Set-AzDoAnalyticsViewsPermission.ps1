Function Set-AzDoAnalyticsViewsPermission
{
    [CmdletBinding()]
    param (
    ...
    Write-Verbose "[Set-AzDoAnalyticsViewsPermission] Started."
    ...
    if (-not $Project)
    {
        Write-Verbose "[Set-AzDoAnalyticsViewsPermission] Project '$ProjectName' not in cache — falling back to live API lookup."
    ...
    {
        throw "[Set-AzDoAnalyticsViewsPermission] Security namespace or project not found. Cannot converge on '$ProjectName'."
    }

    $serializeACLParams = @{
        ReferenceACLs        = $LookupResult.propertiesChanged
        DescriptorACLList    = @()
        DescriptorMatchToken = ($LocalizedDataAzSerializationPatten.AnalyticsViewsPermission -f $Project.id)
    }
    ...
    $projectToken      = '$/Shared/{0}' -f $Project.id
