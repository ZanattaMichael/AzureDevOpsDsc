...
|---|---|---|---|
| [AzDoAreaPermission](Resources/AzDoAreaPermission) | Area Path (CSS namespace) | Multi-identity array | Full org-wide ACL scan; 200-400s per test run — see below |
| [AzDoIterationPermission](Resources/AzDoIterationPermission) | Iteration Path (CSS namespace) | Multi-identity array | Full org-wide ACL scan; 200-400s per test run — see below |
| [AzDoGitPermission](Resources/AzDoGitPermission) | Git repository | Multi-identity array | |
| [AzDoPipelinePermission](Resources/AzDoPipelinePermission) | Build pipeline definition | Single identity + `GroupName` | Full org-wide ACL scan; 200-400s per test run — see below |
| [AzDoProjectPermission](Resources/AzDoProjectPermission) | Project (`$PROJECT` namespace) | Single identity + `GroupName` | |
| [AzDoEnvironmentPermission](Resources/AzDoEnvironmentPermission) | Pipeline environment | Single identity + `GroupName` | |
| [AzDoServiceConnectionPermission](Resources/AzDoServiceConnectionPermission) | Service connection / endpoint | Single identity + `GroupName` | |
| [AzDoAgentPoolPermission](Resources/AzDoAgentPoolPermission) | Agent pool | Single identity + `GroupName` | |
| [AzDoVariableGroupPermission](Resources/AzDoVariableGroupPermission) | Variable group ("library item") | Single identity + `GroupName` | |
| [AzDoArtifactFeedPermission](Resources/AzDoArtifactFeedPermission) | Artifact feed | Role-based (`Reader`/`Collaborator`/`Contributor`/`Administrator`), not bit-based | Different model — see its resource page |
| [AzDoProcessPermission](Resources/AzDoProcessPermission) | Process (`$PROCESS` namespace) | Single identity + `ProcessName` as key | Use `ProcessName = 'AllProcesses'` for the org-wide root scope |
| [AzDoGroupPermission](Resources/AzDoGroupPermission) | Identity/group object itself | Single identity + `GroupName` as key | Controls who can read/edit/delete the *group*, not what the group can do elsewhere |
| [AzDoSecurityNamespacePermission](Resources/AzDoSecurityNamespacePermission) | Any raw CSS Security Namespace + token | Single identity + `GroupName` | Generic escape hatch when no dedicated resource exists for a namespace |
...
- [LCM Configuration](LCMConfiguration) — how to apply permission resources (and everything else) at scale via `AZDO-DSC-LCM`
- [Authentication](Authentication) — the identity your DSC run applies these ACLs *as* needs sufficient rights itself (typically Project Collection Administrator or equivalent namespace-level Administer permission)
