...
| `AzDoOrganizationPolicy` | **Closed (#84).** Implemented as five more properties on `AzDoOrganizationSettings` rather than a new resource: `EnableIPConditionalAccessPolicyValidation`, `LogAuditEvents`, `AllowTeamAdminsToInviteUsers`, `EnableRequestAccess` (+ `RequestAccessUrl`) and `EnableArtifactsFeedUpstreamProtection`, backed by `_apis/OrganizationPolicy/Policies/{policyName}` (separate from the original five properties' `_apis/settings/entries/host` mechanism). `LimitUserVisibility` was left out — it verified as a preview-feature flag, not a confirmed organization policy. Microsoft Entra tenant-level policies (PAT restrictions, organization-creation restrictions) are out of scope here and tracked in #85. |
